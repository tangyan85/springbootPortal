...
public class SecurityConfigCrowd extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        if (ldapConfig.isEnable()) { // 采用ldap            
            ...
            .antMatchers(new String[] {"/js/**", "/vendor/**", "/css/**", "/img/**", "/images/**", "/fonts/**","/**/brand.ico"}).permitAll()
            .anyRequest().authenticated().and().headers().frameOptions().disable().and().formLogin()
...
//            http.authorizeRequests().anyRequest().fullyAuthenticated().and().formLogin().loginPage("/loginLdap")
...
//                                // 维护在session中
//                                request.getSession().setAttribute(SESSION_KEY, user);
//                                response.sendRedirect("idx"); // 通过了就将用户名记录session，并且重定向到idx
//                            }
//                        }
//                    }).permitAll().and().logout().permitAll().and().csrf().disable(); // 暂时禁用CSRF，否则无法提交表单
			...
			.antMatchers("/", "/login").permitAll()
					.antMatchers(new String[] { "/static/**","/js/**", "/vendor/**", "/css/**", "/img/**", "/images/**", "/fonts/**",
							"/**/brand.ico" })
					.permitAll()
					...
					.successForwardUrl("/index").permitAll().failureUrl("/loginError")
            ...
            // .logoutSuccessUrl("/login?logout").permitAll(false)
            ;
        }
    }
    ...
    public CrowdSSOAuthenticationProcessingFilter crowdSSOAuthenticationProcessingFilter() throws Exception {
        CrowdSSOAuthenticationProcessingFilter filter =
                new CrowdSSOAuthenticationProcessingFilter(tokenHelper(), crowdClient(), clientProperties());
