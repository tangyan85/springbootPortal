...
import com.wanda.portal.component.LdapAuthenticationProvider;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        if (ldapConfig.isEnable()) { // 采用ldap
            http.csrf().disable().exceptionHandling().accessDeniedPage("/403").and().authorizeRequests()
                    ...
                    .permitAll().and().headers().frameOptions().disable().and().formLogin()
...
//								response.sendRedirect("index"); // 通过了就将用户名记录session，并且重定向到idx
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/assets/**", "/bower_components/**",
                "/css/**", "/data/**", "/dist/**", "/img/**",
                "/js/**", "/plugins/**", "/vendor/**", "/static/**", "brand.ico");
    }
